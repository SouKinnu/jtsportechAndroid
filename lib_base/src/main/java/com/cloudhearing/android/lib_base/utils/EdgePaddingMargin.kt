...
fun View.removePaddingTopSystemWindowInsets() =
    removeSystemWindowInsetsPadding(removeTop = true)
...
fun View.applySystemWindowInsetsPadding(
    ...
    doOnApplyWindowInsets { view, insets, padding, _ ->
        ...
        Timber.d("apply padding.top:${padding.top} top:$top")
...
fun View.removeSystemWindowInsetsPadding(
    removeLeft: Boolean = false,
    removeTop: Boolean = false,
    removeRight: Boolean = false,
    removeBottom: Boolean = false,
) {
    doOnApplyWindowInsets { view, insets, padding, _ ->
        val systemBars = insets.getInsets(WindowInsetsCompat.Type.systemBars())
        val left = if (removeLeft) systemBars.left else 0
        val top = if (removeTop) systemBars.top else 0
        val right = if (removeRight) systemBars.right else 0
        val bottom = if (removeBottom) systemBars.bottom else 0

        Timber.d("remove padding.top:${padding.top} top:$top")
