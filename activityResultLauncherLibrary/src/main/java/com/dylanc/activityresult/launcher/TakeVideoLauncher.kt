  ...
  private val externalCacheUri: Uri
    get() {
      ...
      return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        FileProvider.getUriForFile(context, "${context.packageName}.result.provider", file)
//        FileProvider.getUriForFile(context, FileProviderUtils.authority ?: "${context.packageName}.result.provider", file)
      } else {
        Uri.fromFile(file)
      }
    }
